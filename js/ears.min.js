      ...
      if(d) {
        ...
        if(z) {
          ...
          (function ga() {
            ...
            q ? (e = setInterval(b, l), f.up.callback(!0)) : k(ga, 50)
          })()
        }
        p = A(a)
      }else {
        q = c, e = setInterval(b, l), z ? (p = parent, f.up.callback(!0)) : (s(a, {props:{src:a.remote + "#" + a.channel + new Date, name:v + a.channel + "_consumer"}, onLoad:function() {
          f.up.callback(!0)
        }}), p = A(a))
      }
    }, init:function() {
...
App.prototype.trackAreaEvent = function() {
  function c(c, d) {
    c.addEventListener(d.event.type, function(c) {
      var g = PG.User.getStory(window.location.href, d.action, d.objects);
      g.objects.x = c.pageX;
      g.objects.y = c.pageY;
      g.objects.text = window.getSelection().toString();
      PG.App.sendToEXDM({url:PG.Cache.protocol + PG.Config.env[PG.Config.mode].url + PG.Config.env[PG.Config.mode].send, method:"POST", data:g, headers:{"Content-Type":"application/json; charset\x3dutf-8"}})
    })
  }
  var d, g;
